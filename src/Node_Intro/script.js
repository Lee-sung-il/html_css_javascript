...
// 여기에 코드를 작성하세요
const ageUsers = users.filter((user) => user.age >= 25 && user.age < 35);
...
function renderUserData(userData) {
  ...
  // 여기에 코드를 작성하세요
  userDataDiv.innerHTML = userData.length
    ? userData
        .map((user) => {
          return `<div>이름 : ${user.name} 나이: ${user.age} 이메일: ${user.email}</div>`;
        })
        .join('')
    : `<p>조건에 맞는 데이터 가 없습니다.</p>`;
}
